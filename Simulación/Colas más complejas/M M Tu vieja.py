...
def tiempos1():
    ...
    sim1.tiempo_ultimo_evento = RELOJ
    if sim1.lista_eventos[0] <= sim1.lista_eventos[1]:
        RELOJ = sim1.lista_eventos[0]
        sim1.proximo_evento = "A"
    else:
        RELOJ = sim1.lista_eventos[1]
...
def tiempos2():
    ...
    if sim2.lista_eventos[0]<sim1.lista_eventos[0]:
        sim2.lista_eventos[0] += global RELOJ
    if sim2.lista_eventos[1]<sim1.lista_eventos[1]:
       sim2.lista_eventos[1] = global RELOJ
    """
    sim2.tiempo_ultimo_evento = RELOJ
    if sim2.lista_eventos[0] <= sim2.lista_eventos[1]:
        RELOJ = sim2.lista_eventos[0]
        sim2.proximo_evento = "A"
    else:
        RELOJ = sim2.lista_eventos[1]
...
def arribo1():
    ...
    if RELOJ != 0:    
        if sim1.lista_eventos[0]<sim2.lista_eventos[0]:
            ...
            RELOJ = sim2.lista_eventos[0]
    sim1.lista_eventos[0] = RELOJ + np.random.exponential(1/sim1.tm_entre_arribos)
    if sim1.estado_servidor == "D":
        sim1.estado_servidor = "O"
        sim1.lista_eventos[1] = RELOJ + np.random.exponential(1/sim1.tm_servicio)
        sim1.ts_acumulado += (sim1.lista_eventos[1] - RELOJ)
...
def arribo2():
    ...
    if RELOJ != 0:    
        if sim1.lista_eventos[0]<sim2.lista_eventos[0]:
            ...
            RELOJ = sim2.lista_eventos[0]
    sim2.lista_eventos[0] = RELOJ + np.random.exponential(1/sim2.tm_entre_arribos)
    if sim2.estado_servidor == "D":
        sim2.estado_servidor = "O"
        sim2.lista_eventos[1] = RELOJ + np.random.exponential(1/sim2.tm_servicio)
        sim2.ts_acumulado += (sim2.lista_eventos[1] - RELOJ)
        sim2.completaron_demora += 1    
        #grafica  
        sim2.relojEnT.append(RELOJ)  
        sim2.cliColaEnT.append(sim2.nro_clientes_cola)
        sim2.tsAcuEnT.append(sim2.ts_acumulado)
    else:
        sim2.area_q_t += sim2.nro_clientes_cola * (RELOJ - sim2.tiempo_ultimo_evento)
        sim2.nro_clientes_cola += 1
        sim2.cola.append(RELOJ)
        #grafica
        sim2.cliColaEnT.append(sim2.nro_clientes_cola)
        sim2.relojEnT.append(RELOJ)
        sim2.tsAcuEnT.append(sim2.ts_acumulado)
...
def partida1():
    ...
    if sim1.nro_clientes_cola > 0:
        sim1.lista_eventos[1] = RELOJ + np.random.exponential(1/sim1.tm_servicio)
        sim1.demora_acumulada += RELOJ - sim1.cola[0]
        sim1.completaron_demora += 1
        sim1.ts_acumulado += sim1.lista_eventos[1] - RELOJ
        sim1.area_q_t += (sim1.nro_clientes_cola * (RELOJ - sim1.tiempo_ultimo_evento))
        sim1.nro_clientes_cola -= 1
...
def partida2():
    ...
    if sim2.nro_clientes_cola > 0:
        sim2.lista_eventos[1] = RELOJ + np.random.exponential(1/sim2.tm_servicio)
        sim2.demora_acumulada += RELOJ - sim2.cola[0]
        sim2.completaron_demora += 1
        sim2.ts_acumulado += sim2.lista_eventos[1] - RELOJ
        sim2.area_q_t += (sim2.nro_clientes_cola * (RELOJ - sim2.tiempo_ultimo_evento))
        sim2.nro_clientes_cola -= 1
